...
app.get('/product/:id', (req, res, next) => {
    // chamada do método do microsserviço
    inventory.SearchProductByID({ id: req.params.id}, (err, product) => {
        // se der erro na comunicação com o microsserviço, retorna para o navegador
        if(err){
            console.error(err);
            res.status(500).send({error: 'algo deu errado :/'});
        }
        else{
            // senão, retorna resultado do microsserviço (JSON) com os dados
